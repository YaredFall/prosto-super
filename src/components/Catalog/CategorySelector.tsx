...
export default function CategorySelector({ selectedCategories = [], setSelectedCategories, onSale = false, setOnSale, className }: Props) {
    ...
    const onCategoryCheckboxChange = (category: GoodsCategories) => (checked: boolean) => {
        setSelectedCategories && setSelectedCategories(checked ? [...selectedCategories, category] : selectedCategories.filter(c => c !== category));
    };
    ...
    return (
        <div className={classNames("flex flex-col gap-1 h-max", className)}>
            ...
            {GOODS_CATEGORIES.map(category => (
                <CategoryCheckItem key={category} label={GoodsCategoriesTranscript.get(category)!} checked={selectedCategories.includes(category)} onChange={onCategoryCheckboxChange(category)} />
            ))}
        </div>
    );
}

function CategoryCheckItem({ label, checked = false, onChange, className }: { label: string, checked?: boolean, onChange?: (newValue: boolean) => void, className?: string; }) {

    return (
        <label className={classNames("py-1 flex items-center gap-2 hover:cursor-pointer group", className)}>
